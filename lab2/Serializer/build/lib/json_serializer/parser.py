def dumps_parser(obj: object):
    return encode(obj)


def dump_parser(obj: object, fp=""):
    tokens = encode(obj)
    fp.write(tokens)


def loads_parser(string: str):
...
def decode(string: str):
    return unpacking_tokens(string)[0]


def unpacking_tokens(string: str):
    ptr = 0
    while ptr < len(string):
        ...
        if string[ptr] == '{':
            ptr += 1
            result = unpacking_dict(string[ptr:])
            ptr += result[1]
            return result[0], ptr
        if string[ptr] == '[':
            ptr += 1
            result = unpacking_list(string[ptr:])
            ptr += result[1]
            return result[0], ptr
        if string[ptr] == '"':
            ptr += 1
            result = unpacking_str(string[ptr:])
            ptr += result[1]
            return result[0], ptr
        if not ptr == len(string) - 1:
            if string[ptr] == '-' and string[ptr + 1].isnumeric():
                ptr += 1
                result = unpacking_nums(string[ptr:])
                ptr += result[1]
...
def unpacking_dict(string: str):
    ...
    while ptr < len(string):
        ...
        if string[ptr] == '}':
            ptr += 1
            break
        result = unpacking_tokens(string[ptr:])
        key = result[0]
        ptr += result[1]
